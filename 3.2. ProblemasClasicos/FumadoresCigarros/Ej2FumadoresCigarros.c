/**
 * @file Ej2FumadoresCigarros.c
 * @brief Solución al problema de los fumadores con semáforos y hilos.
 ...
 *        los toma, forma su cigarro y fuma.
...
void* fumadorT(void* arg) {
    while (1) {
        ...
        if (sem_trywait(&papel) == 0) {
            // Tiene fósforo y papel, puede fumar
            printf("Fumador T (tiene tabaco) arma el cigarro y fuma.\n");
            usleep(100000);  // Simula fumar
            sem_post(&producir);
        } else {
            // No consiguió el segundo recurso, regresa el primero
            sem_post(&fosforo);
        }
    }
    return NULL;
